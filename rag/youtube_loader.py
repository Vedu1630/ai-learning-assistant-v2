...
def load_youtube(url, title=None, proxy=None):
    ...
    # Tier 1: Try native yt-dlp first ONLY if NOT on Render
    if not is_render:
        try:
    ...
    if not transcript:
        try:
            import re
            print("Fetching transcript from third-party API...")
            ...
            # If native failed, output both errors, else just fallback error
            if not is_render:
                raise Exception(f"Could not retrieve transcript. Native: {e}. Fallback: {fallback_err}")
            else:
                raise Exception(f"Could not retrieve transcript from third-party API: {fallback_err}")
